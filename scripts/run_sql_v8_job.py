"""Run one V8 Arctic Q4 evidence ablation under the one-hour ledger."""
import argparse
import sys
from pathlib import Path

from llm_evals import ComputeBudget

parser = argparse.ArgumentParser()
parser.add_argument("--job-id", required=True)
parser.add_argument("--mode", choices=["generated_literals", "generated_descriptions",
                                      "generated_normalization", "generated_all"], required=True)
parser.add_argument("--scope", choices=["pilot", "development"], default="pilot")
parser.add_argument("--estimate-seconds", type=float, default=500)
parser.add_argument("--reserve-seconds", type=float, default=0)
parser.add_argument("--artifacts", type=Path, default=Path(__file__).resolve().parents[2] / "artifacts")
args = parser.parse_args()
artifacts = args.artifacts.resolve()
budget = ComputeBudget(artifacts / "v8/compute", limits={"sql-agent": 3600}, total_seconds=3600)
command = [sys.executable, str(Path(__file__).with_name("run_sql_v8_eval.py")),
           "--artifacts", str(artifacts), "--mode", args.mode, "--scope", args.scope]
print(budget.run(args.job_id, "sql-agent", command, cwd=Path(__file__).resolve().parents[1],
                 estimate_seconds=args.estimate_seconds, reserve_seconds=args.reserve_seconds))
