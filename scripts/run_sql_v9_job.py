"""Run one V9 model job under the frozen one-hour sequential ledger."""
import argparse
import sys
...
parser.add_argument("--mode", choices=["p0_cache_v2", "scoped", "probed"], required=True)
...
budget = ComputeBudget(artifacts / "v9/compute", limits={"sql-agent": 3600}, total_seconds=3600)
command = [sys.executable, str(Path(__file__).with_name("run_sql_v9_eval.py")),
