...
def _use_recorded_asset_digests() -> None:
    ...
    recorded = json.loads((REMOTE / "v11/compute/modal-asset-digests.json").read_text(encoding="utf-8"))["assets"]
...
def gpu_job(command: str, recipe: str = "A", attempt_id: str = "modal-a100-2") -> dict:
    ...
    try:
        if command == "preflight":
            result = v11_training.preflight(REMOTE, attempt_id=attempt_id)
            adapter = REMOTE / f"v11/sql-agent/preflight/{attempt_id}/adapter"
            result["reload"] = v11_training.reload_check(REMOTE, adapter)
        elif command in {"pilot", "full"}:
            result = v11_training.train(REMOTE, recipe, pilot=command == "pilot")
            final = REMOTE / f"v11/sql-agent/training/{command}/{recipe}/final"
            result["reload"] = v11_training.reload_check(REMOTE, final)
...
def _link_databases(prepared: Path | None) -> None:
    ...
    sources = [REMOTE / "v11/sql-agent/data/development.jsonl"] + ([prepared] if prepared else [])
...
def evaluate(stage: str, variant: str) -> dict:
    ...
    volume.reload()
    prepared = REMOTE / "v11/sql-agent/data/eval-regression.jsonl"
    _link_databases(prepared if stage == "regression" else None)
    started = time.time()
    try:
        report = v11_evaluation.run(REMOTE, stage, variant)
...
def main(stage: str, recipe: str = "A", variants: str = ""):
    ...
    if stage in {"preflight", "pilot", "full"}:
        ...
        with volume.batch_upload(force=True) as batch:
            batch.put_file(str(digests), "/v11/compute/modal-asset-digests.json")
    if stage == "setup":
        _upload_inputs()
        ...
        decision = LOCAL_ARTIFACTS / "v11/sql-agent/pilot-decision.json"
        if not decision.is_file():
            raise SystemExit("Freeze the pilot decision locally before full training.")
        with volume.batch_upload(force=True) as batch:
            batch.put_file(str(decision), "/v11/sql-agent/pilot-decision.json")
        ...
        pulled = (_pull("v11/sql-agent/training") + _pull("v11/sql-agent/preflight")
                  + _pull("v11/sql-agent/eval"))
        exposure = LOCAL_ARTIFACTS / "v11/compute/modal-exposure.jsonl"
        exposure.write_bytes(b"".join(volume.read_file("v11/exposure.jsonl")))
